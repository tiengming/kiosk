<script lang="ts">
   ...
   * positive grade.
   */
  export let grade: number | `${number}` = 0;
   ...
   * the symbol.
   */
  export let weight: number | `${number}` = 400;
   ...
   * the symbol size.
   */
  export let opticalSize: number | `${number}` = 48;
...
<span class="icon material-symbols-outlined { className }" style="{styles}" aria-hidden="true">
  <slot>{ name }</slot>
</span>
